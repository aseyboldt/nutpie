...
By default, `nutpie` initializes each chain using PyMC's own initial point
strategy (`support_point` or `prior`, see `default_initialization_strategy`
above). You can override this by providing a function that computes the
initial, unconstrained point for a chain given an integer seed:
...
Calling `with_init_point_fn` again replaces any previously configured
callback (including the default one), it never combines with it. This
behaves the same way for all backends (PyMC, Stan and custom `from_pyfunc`
models).
